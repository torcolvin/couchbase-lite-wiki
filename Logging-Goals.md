This document is going to attempt to capture some guidelines and rules for implementing logging that doesn't miss the mark.  Too often we receive logs only to realize that logging is not adequate exactly in the part that we need it to be, but instead there are hundreds of other messages that provide no helpful information in the context.  If logging is going to be a reliable part of the debugging process going forward, then it needs to be standardized.  First let's start with the logging levels:
